...
- **Fase 2b — Auth: ✅ FEITA E VALIDADA (2026-06-26).** `verify_google_token` + `upsert_google_user` em `auth.py`; endpoints `GET /api/auth/config` e `POST /api/auth/google`; config `GOOGLE_CLIENT_ID` + `GOOGLE_ALLOWED_DOMAINS` (gate de domínio opcional). Frontend: botão Google Identity Services no `index.html`/`app.js` (aparece só se houver client_id; login usuário/senha continua como fallback). Sem isolamento — todos veem tudo. Validado: upsert cria/atualiza sem duplicar, guarda google_sub, gate de domínio bloqueia, endpoints respondem, login usuário/senha intacto. Handoff: o usuário criou o OAuth Client ID, autorizou as origens e setou `GOOGLE_CLIENT_ID`/`GOOGLE_ALLOWED_DOMAINS` no `.env`. **Entrega: login Google multi-usuário (visão compartilhada).**
- **Fase 2b+ — Google-only + UX: ✅ FEITA E TESTADA NO NAVEGADOR (2026-06-26, commit `47a75af`).**
  - **Google-only:** removido o login usuário/senha (back: `verify_credentials`, rota `/auth/login`, config `auth_user/password`; front: campos user/pass/btn-login). Fecha o hardening das credenciais `admin/lotepro` no HTML. `LOTEPRO_SECRET` segue assinando o token.
  - **Reabrir projeto salvo:** `GET /api/projects/{pid}/load` reconstrói o payload da análise (results+report+center+lots_info) do banco, com geometrias **simplificadas** (`simplify(0.00001)`) — senão um projeto de cidade vira 100+ MB. Dropdown "Projetos salvos" no frontend, atualizado no login e após cada análise.
  - **Sessão persistente:** token salvo em `localStorage` e restaurado no reload via `restoreSession()` (valida com uma rota protegida; 401 → cai no login). Botão **"Sair"** no header.
  - **Loading:** spinner visível abaixo do select ao abrir projeto; ajuste de CSS do botão de logout.
  - **Estudo de IA (Motor de Bolhas) persiste:** `POST /api/bolha/analyze` grava `estudo` em `lot_crm.bolha` (JSONB) automaticamente (`routes.py:154`). Ao reabrir o lote, `openBolha` mostra o estudo salvo **sem re-chamar a IA** → não gasta token de novo.

## ▶️ Onde paramos / próximo passo

Tudo de Fase 2 concluído e testado. Falta apenas **push da branch `feat/fase2-postgres-google-auth` + PR** e o `.env` de produção (Google OAuth + segredos + origem https autorizada). Status consolidado no topo de `../PLANO_MIGRACAO_AMBGEO_LOTEPRO.md`.

**Follow-ups conhecidos:** popular `created_by`/`updated_by`; re-vincular CRM por correspondência espacial ao re-detectar; (opcional) mover caches globais (`msbuildings/`, `dem_cache.json`) para banco/volume.
